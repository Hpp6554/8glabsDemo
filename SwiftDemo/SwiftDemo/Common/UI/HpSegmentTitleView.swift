...
@objc protocol HpSegmentTitleViewDelegate: NSObjectProtocol {
    @objc optional func segmentTitleView(_ page: HpSegmentTitleView, at index: Int)
}

class HpSegmentTitleView: HpBaseView {
    public weak var delegate: HpSegmentTitleViewDelegate?
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collectionView.register(HpSegmentTitleCell.self, forCellWithReuseIdentifier: cellID)
    ...
    override func fm_addSubviews() {
        super.fm_addSubviews()
...
fileprivate extension HpSegmentTitleView {
    /// 更新配置
        ...
        if self.config.positionStyle != .none {
            self.collectionView.performBatchUpdates({
                ...
                if let strongSelf = self {
                    strongSelf.collectionView.sendSubviewToBack(strongSelf.indicator)
                }
    ...
    /// 获取指示器位置
    func rectIndicator(cell: HpSegmentTitleCell, index: Int) -> CGRect {
    ...
    /// 设置选中项
    func setSelectedCell(cell: HpSegmentTitleCell?, at indexPath: IndexPath, animated: Bool) {
        self.selectedIndex = indexPath.item
        let seleIndexPath = IndexPath(item: self.selectedIndex, section: 0)
        let seleCell = self.collectionView.cellForItem(at: seleIndexPath) as? HpSegmentTitleCell
        for subCell in self.collectionView.visibleCells {
            if subCell == cell {
                (subCell as? HpSegmentTitleCell)?.updateCellTitle(isChecked: true, animated: animated)
            }
            else if subCell == seleCell {
                (subCell as? HpSegmentTitleCell)?.updateCellTitle(isChecked: false, animated: animated)
            }
            else {
                (subCell as? HpSegmentTitleCell)?.updateCellTitle(isChecked: false, animated: false)
            }
        }
    ...
    func setSelectedCell(at index: Int, animated: Bool) {
        ...
        let cell = collectionView.cellForItem(at: indexPath) as? HpSegmentTitleCell
        self.setSelectedCell(cell: cell, at: indexPath, animated: animated)
    }
    /// 设置滚动条
    func setSelectedIndicator(cell: HpSegmentTitleCell, at index: Int, animated: Bool) {
    ...
    func scrollTo(selectIndex: Int, willSelectIndex: Int, progress: CGFloat) {
        ...
        let seleCell = collectionView.cellForItem(at: seleIndexPath) as? HpSegmentTitleCell
        let willCell = collectionView.cellForItem(at: willIndexPath) as? HpSegmentTitleCell
...
extension HpSegmentTitleView: UICollectionViewDataSource {
    // MARK: - UICollectionViewDataSource
    public func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return self.titles.count
    }
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell: HpSegmentTitleCell = collectionView.dequeueReusableCell(withReuseIdentifier: cellID, for: indexPath) as! HpSegmentTitleCell
...
extension HpSegmentTitleView: UICollectionViewDelegateFlowLayout {
